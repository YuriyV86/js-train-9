...
function findElementGreaterThan(arr, num) {
  ...
  if (Array.isArray(arr)) {
    let a = arr.find(value => value > num);
    if (a) {
      return a;
    }
    else {
      console.log(`Числа яке задовільняе умову не знайдено`);
    }
  }
  else {
    return undefined;
...
function getPositiveArrayValues(arr) {
  ...
  if (Array.isArray(arr)) {
    return Array.from(arr.values()).filter(elem => elem > 0);
  }
  else {
    return `Вхідний параметр має бути масивом`;
  } 
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  if (Array.isArray(arr)) {
    return arr.flat().map(elem => elem * elem).filter(element => element > threshold);
  }
  else {
    return `Вхідний параметр має бути масивом`;
  } 
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  if (Array.isArray(arr)) {
    ...
    return arr2.forEach(x => console.log(x)), arr2.reduceRight((sum, x) => x + sum, 0);
  }
  else {
    return `Вхідний параметр має бути масивом`;
  } 
...
function expandArrayByN(arr, n) {
  ...
  if (Array.isArray(arr) && (typeof n === 'number')) {
   let expandedArray = arr.flatMap(x => 
...
function findLongestWord(arr) {
  ...
  if (Array.isArray(arr)) {
    return arr.reduce((maxWord, elem) => (elem.length > maxWord.length) ? elem : maxWord , '');
  }
  else {
    return ``;
  } 
...
function findDuplicateElements(arr) {
  ...
  if (Array.isArray(arr)) {
    return arr.filter((elem, idx) => {return idx !== arr.indexOf(elem)});
  }
  else {
    return [];
  } 
...
function capitalizeWords(arr) {
  ...
  if (Array.isArray(arr)) {
    return arr.map(elem => {return elem[0].toUpperCase() + elem.substring(1)});
  }
  else {
    return [];
  } 
...
/**
  ...
  Використовується метод () для реалізації.
...
function calculateTotalPrice(arr) {
  ...
  // Використовуємо метод () для обчислення загальної ціни
  // Додаємо ціну товару до загальної суми та повертаємо результат
  // Повертаємо загальну ціну всіх товарів
  if (Array.isArray(arr)) {
    return arr.reduce((sum, elem) => {return sum + elem.price}, 0);
  }
  else {
    return 0;
  } 
